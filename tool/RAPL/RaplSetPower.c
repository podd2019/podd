...
*/
	...
	rdmsr(fd, MSR_PKG_RAPL_POWER_LIMIT, &reg);
        printf("%" PRIu64 "\n", reg);
	int cpu=0;
        printf("cpu%d: ENABLE_POWER_LIMIT: (%sabled)\n",cpu, (reg >> 15) & 1 ? "EN": "DIS");
        printf("cpu%d: MSR_PKG_RAPL_POWER_LIMIT: (%slocked)\n",cpu, (reg >> 63) & 1 ? "": "UN");
	reg = (reg & 0xFFFFFFFFFFFF0000) | setpoint | 0x8000;
	reg = (reg & 0xFFFFFFFF0000FFFF) | 0xD0000;
	wrmsr(fd, MSR_PKG_RAPL_POWER_LIMIT, reg);
...
int main(int argc, char **argv) {
  ...
  double power_target2 = atoi(argv[2]);
  printf("Starting set 1 and 2 package power to %fW %fW\n",power_target1,power_target2);
  while (core < 2){
...
// set power limit 30
 // printf("\n");
  if (core == 0){
      if (power_target1 != 0){
          set_power_limit(fd, power_target1, power_units);
      }  
  }
  else{
      if (power_target2 != 0 ){
          set_power_limit(fd, power_target2, power_units);
      }
  }
 // uint64_t currentval, newval, mask = 0, offset = 0;
 // currentval=read_msr(fd,MSR_PKG_RAPL_POWER_LIMIT);  
 // printf("BEFORE CHANGE:\n");
 // printf("RAPL power limit = %lx\n", currentval);
 // printf("RAPL power limit = %.6fW\n", power_units*(double)(currentval&0x7fff));
